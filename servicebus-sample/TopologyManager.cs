    ...
    public class TopologyManager
    {
        ...
        private async Task EnsureOrderQueueAsync()
        {
            if ((await _administrationClient
                    .QueueExistsAsync(OrderQueuePath)
        ...
        {
            if ((await _administrationClient
                    .TopicExistsAsync(ShipmentTopicPath)
                    .ConfigureAwait(false)).Value)
            {
                return;
            }
        ...
        private async Task EnsureSubscriptionAsync(string name, string sqlFilter)
        {
            ...
            if (string.IsNullOrEmpty(sqlFilter))
            {
                await _administrationClient
                    .CreateSubscriptionAsync(options)
        ...
        public async Task DeleteTopologyAsync()
        {
            ...
            {
                await _administrationClient
                    .DeleteQueueAsync(OrderQueuePath)
                    .ConfigureAwait(false);
            }
            ...
            {
                await _administrationClient
                    .DeleteTopicAsync(ShipmentTopicPath)
                    .ConfigureAwait(false);
            }
        }
    }
